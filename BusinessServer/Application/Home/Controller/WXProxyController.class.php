...
class WXProxyController extends Controller {
    ...
    public function myProxyList(){
        ...
        if(empty($openID)){
            ...
            if(!empty($code)){
                ...
                header("Location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=".WX_APPID."&redirect_uri=".HTTP_PROXY_URL_PREFIX."myProxyList&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect");
                exit;
            }
        }
    ...
    public function myTradeList(){
        ...
        $sql = 'SELECT pt.amount, pt.add_date, p.name FROM biz_proxy_trade pt LEFT JOIN biz_proxy_info p ON(pt.lower_proxy_id=p.user_id)
